...
import UsedByTable from '../components/UsedByTable.vue';
import { notionUsedBy, snapshot } from '../data/snapshot';
import type { ConsumerKind } from './glossary-consumers';
import { filterNotions, groupNotions } from './page-data';

const query = ref('');
const owner = ref('');
const usedByKind = ref<'' | ConsumerKind>('');
const owners = computed(() => [...new Set(snapshot.value.snapshot.notions.map((notion) => notion.owner))].sort());
const filteredNotions = computed(() =>
  filterNotions(snapshot.value.snapshot.notions, query.value)
    .filter((notion) => owner.value === '' || notion.owner === owner.value)
    .filter(
      (notion) =>
        usedByKind.value === '' ||
        notionUsedBy.value
          .get(notion.name)
          ?.some((consumer) => consumer.kind === usedByKind.value) === true,
    ),
);
        ...
        >
          ...
          >
            ...
            <div class="glossary-card__consumers">
              <h4>Used by</h4>
              <UsedByTable
                v-if="notionUsedBy.get(notion.name)?.length"
                :consumers="notionUsedBy.get(notion.name) ?? []"
                :kind-filter="usedByKind"
              />
...
.glossary__filter { display: grid; gap: 0.35rem; margin-top: 0.75rem; color: var(--color-text-muted); font-size: 0.8125rem; }
.glossary__filter select { width: 100%; min-height: 2.5rem; padding: 0.5rem; border: 1px solid var(--color-border); border-radius: 0.4rem; color: var(--color-text); background: var(--color-surface); }
...
.glossary__toc-group ul { margin: 0; padding: 0; list-style: none; }
